/**
 * DespesasPage.jsx — Seletor de Movimentos Bancários (Saídas) para Reconciliação
 */
...
export default function DespesasPage() {
  ...
  const { prefs, setFilter, resetFilters } = useFilterPrefs(empresaId, 'saidas_extratos', FILTER_DEFAULTS);
  ...
  const hasFilters = prefs.tipoExtrato !== 'débito' || prefs.search;
  ...
  return (
    <div className="contas-page">
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', padding: '16px 24px 8px 24px' }}>
        <div>
          <button onClick={() => navigate(`/empresa/${empresaId}/despesas`)} className="btn btn-secondary" style={{ padding: '6px 12px', fontSize: 13, display: 'flex', gap: 6, alignItems: 'center', border: '1px solid var(--color-border)', width: 'fit-content', marginBottom: 12 }}>← Voltar</button>
          
          <h1 style={{ fontSize: 24, fontWeight: 700, margin: 0, display: 'flex', alignItems: 'center', gap: 10 }}>
            💸 Triagem de Saídas
          </h1>
          <p style={{ color: 'var(--color-text-muted)', margin: '4px 0 0 0', fontSize: 13 }}>
            Selecione as movimentações bancárias de saída (débitos) e envie-as para a esteira de reconciliação documental.
          </p>
        </div>
